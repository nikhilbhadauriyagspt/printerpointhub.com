    ...
    <div className="flex h-screen bg-gray-50 font-urbanist">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-slate-900 text-white transition-transform duration-300 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:relative lg:translate-x-0`}>
        <div className="flex items-center justify-between h-16 px-6 border-b border-slate-800">
                     <span className="text-xl font-black tracking-tighter">PRINTER POINT HUB<span className="text-indigo-500">.</span> ADMIN</span>          <button onClick={() => setSidebarOpen(false)} className="lg:hidden text-slate-400 hover:text-white">
            <X size={20} />
          </button>
        </div>
        ...
        <header className="flex items-center justify-between h-16 px-6 bg-white border-b border-gray-200 shadow-sm">
          ...
          <div className="flex items-center gap-6 ml-auto">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="h-9 w-9 bg-blue-100 text-indigo-600 rounded-full flex items-center justify-center font-bold">
                {admin?.name?.charAt(0)}
              </div>
            </div>
          </div>
