  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        ...
        <Link to="/admin/products/add" className="bg-indigo-600 hover:bg-blue-700 text-white px-6 py-3 rounded-xl font-bold text-sm flex items-center gap-2 transition-all shadow-lg shadow-indigo-600/20">
            ...
            <tbody className="divide-y divide-gray-100">
              {loading ? (
                ...
                <tr key={p.id} className="hover:bg-gray-50/50 transition-colors group">
                  ...
                  <td className="px-6 py-4">
                    <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${p.status === 'published' ? 'bg-indigo-50 text-indigo-600' : 'bg-gray-100 text-gray-400'}`}>
                      {p.status}
                    </span>
                  </td>
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <Link to={`/admin/products/edit/${p.id}`} className="p-2 hover:bg-indigo-50 text-indigo-600 rounded-lg transition-colors">
                        <Edit size={16} />
                      </Link>
                      <button 
