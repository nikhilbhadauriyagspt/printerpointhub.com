  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-3xl font-black text-slate-900 tracking-tighter uppercase">Order Management</h1>
          <p className="text-sm font-bold text-slate-500 uppercase tracking-widest mt-1">Manage and track customer purchases</p>
        </div>
                ...
                <tr key={order.id} className="hover:bg-gray-50/50 transition-colors group">
                  <td className="px-8 py-6">
                    <div className="flex flex-col">
                      <span className="text-sm font-black text-slate-900">#PFX-{order.id}</span>
                      <span className="text-[10px] font-bold text-slate-400 uppercase mt-1">{new Date(order.created_at).toLocaleDateString()}</span>
                    </div>
                  </td>
                  <td className="px-8 py-6">
                    <div className="flex flex-col">
                      <span className="text-sm font-black text-slate-900 uppercase">{order.first_name} {order.last_name}</span>
                      <span className="text-[10px] font-bold text-slate-400 uppercase mt-1">{order.guest_email}</span>
                    </div>
                  </td>
                  <td className="px-8 py-6">
                    <div className="flex flex-col">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-4">
                    ...
                    <div className="bg-gray-50 rounded-2xl p-6">
                      <p className="text-sm font-black text-slate-900 uppercase">{selectedOrder.first_name} {selectedOrder.last_name}</p>
                      ...
                      <div className="flex items-center gap-2 text-xs font-bold text-slate-500 mt-1 uppercase"><Phone size={12} /> {selectedOrder.phone}</div>
                    </div>
                  </div>
                  <div className="space-y-4">
                    <div className="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">
                    ...
                    {selectedOrder.items?.map((item, idx) => (
                      <div key={idx} className="p-4 flex items-center justify-between bg-white">
                        <div className="flex items-center gap-4">
                          <div className="h-10 w-10 rounded-lg bg-gray-50 flex items-center justify-center text-[10px] font-black">
                            {item.quantity}x
                          </div>
                ...
                <div className="pt-6 border-t border-gray-100">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="space-y-4">
                      ...
                      <div className="bg-slate-50 rounded-2xl p-6 border border-slate-100">
                        <div className="flex justify-between mb-2">
                          ...
                          <span className="text-[10px] font-black text-slate-900 uppercase">{selectedOrder.payment_method}</span>
                        </div>
                          ...
                          <div className="space-y-2 mt-4 pt-4 border-t border-slate-200">
                            {(() => {
                              try {
                                ...
                                if (!details) return <p className="text-[9px] text-slate-400 uppercase italic">No transaction data</p>;
                                ...
                                return (
                                  <>
                                    ...
                                    <div className="flex justify-between">
                                      ...
                                      <span className="text-[9px] font-black text-slate-900">{details.payer?.email_address || details.email || 'N/A'}</span>
                                    </div>
                                    <div className="flex justify-between">
                                      <span className="text-[9px] font-bold text-slate-400 uppercase">Status</span>
                                      <span className="text-[9px] font-black text-emerald-600 uppercase">{details.status || 'COMPLETED'}</span>
                                    </div>
                                  </>
                                );
                              } catch (e) {
                                return <p className="text-[9px] text-slate-400 uppercase italic">Invalid data format</p>;
                              }
                            })()}
                          </div>
                        )}
                    ...
                    <div className="flex flex-col justify-end text-right">
                      <span className="text-sm font-black text-slate-400 uppercase">Total Amount</span>
                      <span className="text-4xl font-black text-slate-900">${selectedOrder.total_amount}</span>
                    </div>
                  </div>
                </div>
              </div>
