  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-3xl font-black text-slate-900 tracking-tighter uppercase">Inquiry Manager</h1>
          <p className="text-sm font-bold text-slate-500 uppercase tracking-widest mt-1">Manage customer messages and support tickets</p>
        </div>
                ...
                <tr key={contact.id} className="hover:bg-gray-50/50 transition-colors group">
                  <td className="px-8 py-6">
                    <div className="flex flex-col">
                      <span className="text-sm font-black text-slate-900 uppercase">{contact.name}</span>
                      <span className="text-[10px] font-bold text-slate-400 lowercase mt-1">{contact.email}</span>
                    </div>
                  </td>
                  <td className="px-8 py-6">
                    <span className="text-xs font-bold text-slate-600 uppercase tracking-tight line-clamp-1">{contact.subject}</span>
                  </td>
                  <td className="px-8 py-6">
                  ...
                  <td className="px-8 py-6">
                    <span className="text-[10px] font-bold text-slate-400 uppercase">{new Date(contact.created_at).toLocaleDateString()}</span>
                  </td>
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-8 mb-10">
                <div className="space-y-1">
                  ...
                  <p className="text-sm font-black text-slate-900 uppercase">{selectedContact.name}</p>
                ...
                <div className="space-y-1 text-right">
                  ...
                  <p className="text-sm font-black text-slate-900 uppercase">{selectedContact.subject}</p>
                  <p className="text-xs font-bold text-slate-500 uppercase">{new Date(selectedContact.created_at).toLocaleString()}</p>
                </div>
              </div>

              <div className="bg-gray-50 rounded-[2rem] p-8 mb-10 border border-gray-100">
                <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-4">Message</p>
